## summary function
#' summary function for GBOP2_minSS_single, GBOP2_maxP_single, GBOP2_minSS_dual, PSO_design, PSO_power, PSO_design_dual,PSO_power_dual, PSO_design_TE, PSO_power_TE
#'
#' @param object GBOP2_minSS_single, GBOP2_maxP_single, GBOP2_minSS_dual, PSO_design, PSO_power, PSO_design_dual,PSO_power_dual, PSO_design_TE, PSO_power_TE
...
summary.gbop2 <- function(object, ...) {
  ...
  ########## GBOP2_minSS_single ########################################
  if (object$`function` == "GBOP2_minSS_single") {
    # Extract relevant fields from object
    design <- object$design
    weight <- object$weight
    method <- object$method
  ...
  ########## GBOP2_maxP_single ########################################
  if (object$`function` == "GBOP2_maxP_single") {
    # Extract relevant fields from object
    design <- object$design
    method <- object$method
    cputime <- object$cputime
  ...
  if (object$`function` == "GBOP2_minSS_dual") {
    ...
    boundary1 <- c(object$cohort1bd1, object$cohort2bd1)
    boundary2 <- c(object$cohort1bd2, object$cohort2bd2)
 ...
 ##################GBOP2_maxP_dual######################
  if (object$`function` == "GBOP2_maxP_dual") {
  design <- object$design
  weight <- object$weight
  method <- object$method
  lambda1 <- object$parameter.lambda1
  lambda_grad1 <- object$parameter.lambda_grad1
  lambda_grad2 <- object$parameter.lambda_grad2
  gamma_1 <- object$parameter.gamma_1
  gamma_2 <- object$parameter.gamma_2
  gamma_3 <- object$parameter.gamma_3
  delta0 <- object$parameter.delta0
  delta1 <- object$parameter.delta1
...
#######################GBOP2_minSS_TE#################
  if (object$`function` == "GBOP2_minSS_TE") {
    ...
    gamma <- object$gamma
    cohort <- c(object$cohort1, object$cohort2)  # Cohort sizes
    boundary_effi <- c(object$boundary_effi1, object$boundary_effi2)  # Efficacy boundaries
    boundary_toxi <- c(object$boundary_toxi1, object$boundary_toxi2)  # Toxicity boundaries
    ...
    cat("Boundary Values:\n")
    cat("  Efficacy:        ", paste(boundary_effi, collapse = ", "), "\n")
    cat("  Toxicity:        ", paste(boundary_toxi, collapse = ", "), "\n\n")
  ...
  if (object$`function` == "PSO_design_TE") {
    ...
    cat("Expected Sample:   ", ifelse(!is.null(expected_sample), 
                                      format(expected_sample, digits = 6), "N/A"), "\n")
