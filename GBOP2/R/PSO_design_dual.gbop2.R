#' PSO: Optimal/Minimax design with dual boundaries
#'
#' This function implements Particle Swarm Optimization (PSO) to find an optimal or minimax design with  dual decision boundaries.
#'
...
PSO_design_dual <- function(
  ...
  objf <- function(x, inputlist, fcn) {
    ...
    # }
      ...
      w_list <- numeric(n_cohort)
      w_list[1] <- (cos(theta[1]))^2  # First weight
      ...
      w_list[n_cohort] <- (prod(sin(theta[1:(n_cohort - 1)])))^2  # Last weight
    }
    ...
    temp=.Call("_GBOP2_GetocBiRcpp_dual", seed=inputlist$seed, nsim=numOfSimForTiralSetting,
    ...
    if (t1e>inputlist$err1 | power<inputlist$minPower){
      results = 999
        ...
        if(design =="optimal"){ ## optimal
          ...
          n_final = nobs.seq[length(nobs.seq)] ## n_final is total sample size
          results = unified.u*n_final + (weight*temp[[4]] + (1-weight)*temp[[5]])/n_final
        }
    }
  ...
  for ( i in n_sim){
    ...
    power_result = .Call("_GBOP2_GetocBiRcpp_dual", seed=input$seed, nsim=numOfSimForTiralSetting,
  ...
  results_list <- list(
    ...
    "cohort" = as.list(nobs2),"boundary"=list("1" = bd[, 2], "2" = bd[, 3]), ## bd2 is first boundary, bd3 is second boundary
