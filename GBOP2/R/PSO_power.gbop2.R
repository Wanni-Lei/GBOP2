#' PSO: Power maximizing design with single boundary
#'
#' This function implements Particle Swarm Optimization (PSO) to find an power maximizing design with single boundary.
#'
...
PSO_power <- function(
  ...
  objf <- function(x, inputlist, fcn) { ## x is parameter set
    ...
    temp=.Call("_GBOP2_GetocBiRcpp", seed=inputlist$seed, nsim = input$numOfSimForTiralSetting,
                     contrast=inputlist$contrast, nobs=nobs.seq,
                     b = b, b2 = b2, pow2 = pow, dprior = inputlist$dprior, ptrue = inputlist$b1a, phi = inputlist$b1n, fff=fcn);
  ...
  for ( i in 1:n_sim){
    ...
    results_list <- list("function" = "PSO_power", "design" = design, "method" = method,
                        "cputime" = res$cputime,
